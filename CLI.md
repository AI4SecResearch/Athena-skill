...
`athena` 直接跑 `athena.py`(标准库,无依赖)。当前只用 `ask`,取 pid 用 `projects`,其余命令暂时屏蔽。命令↔API 映射见 `athena.py` 头部注释。
...
```bash
A=athena-skill/athena.py; $A projects; $A ask "<问题>"
```
...
project 没设就先 `athena projects` 拿 id。server 必须在跑。

# 注

- `ask`:stdout 出结果,`task_id=...` 到 stderr(轮询在 CLI 内),答案末尾有 `## 证据链`。
- 4xx→exit 1、5xx→exit 2,`detail` 到 stderr;server 没起,`athena projects` 打印"服务未运行"。
